...
import { FileText, BookOpen, CheckCircle, XCircle, ArrowRight, Timer } from "lucide-react";
      ...
      <DashboardLayout>
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-4">
            <Button variant="ghost" size="sm" onClick={resetTest}>← Ortga</Button>
          ...
          <div className="flex justify-end mt-4">
            ...
            {isRevealed && currentQ === totalQ - 1 && (
              <Button size="sm" onClick={resetTest}>Yakunlash <CheckCircle className="w-4 h-4 ml-1" /></Button>
            )}
          </div>
        </div>
      </DashboardLayout>
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Biletni tanlang va mashq qiling (vaqt chegarasisiz)</p>
      </div>
      ...
      {(!tickets || tickets.length === 0) && (
        <div className="text-center py-12 text-muted-foreground"><BookOpen className="w-10 h-10 mx-auto mb-2 opacity-30" /><p className="text-sm">Hali biletlar yo'q</p></div>
      )}
    </DashboardLayout>
  );
}
